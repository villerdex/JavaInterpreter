...
public class Parser {
    ...
    public void parse(ArrayList<Token> tokenList) {
        ...
        while (getCurrentToken().getType() != Token.Type.END_BLOCK) {
            //System.out.println(getCurrentToken().getText() + "Current Token: " + currentTokenIndex);
    ...
    public void identifyKeyWord(Token.Type token) {
        Statement statement;
        switch (token) {
            case VARIABLE:
            ...
            case PRINT:
                ...
                break;
            case IF:
                statement = parselet.statementIF();
                statementsAST.add(statement);
                break;
        }
    }

    public Statement getStatementByKeyWord(Token.Type token) {
